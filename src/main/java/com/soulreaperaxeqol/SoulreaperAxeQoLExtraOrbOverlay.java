...
@Slf4j
public class SoulreaperAxeQoLExtraOrbOverlay extends Overlay {
    ...
    public void updateLayer(boolean alwaysOnTop)
    {
        setLayer(alwaysOnTop
    ...
    @Inject
    public SoulreaperAxeQoLExtraOrbOverlay(Client client, SoulreaperAxeQoLPlugin plugin, SoulreaperAxeQoLConfig config, ConfigManager configManager) {
    ...
    public Dimension render(Graphics2D g) {
        ...
        if (plugin.isSoulreaperAxeEquipped()) {
            ...
            renderSpecOrbSprite(g,(double) plugin.getSoulreaperStackCount() / (double) SOULREAPER_MAX_STACKS, orbImage);
        }
        ...
        if (plugin.isSoulreaperAxeEquipped()) {
            ...
            renderExtraOrbRegenTimer(g, plugin.getSpecRegenProgress(), config.getSpecRegenColor(), true);
        } else {
            if (!config.showTextOnOrb()) {
                renderOrbIcon(g, soulreaperSprite);
                renderText(g, String.valueOf(plugin.getSoulreaperStackCount()), true, (double) plugin.getSoulreaperStackCount() / (double) SOULREAPER_MAX_STACKS, 0, 5);
            } else {
                renderText(g, String.valueOf(plugin.getSoulreaperStackCount()), true, (double) plugin.getSoulreaperStackCount() / (double) SOULREAPER_MAX_STACKS, 24, 3);
            }
            renderExtraOrbRegenTimer(g, plugin.getSoulreaperRegenProgress(), config.getSoulreaperRegenColor(), true);
        }
    ...
    private void renderExtraOrbRegenTimer(Graphics2D g, double percent, Color color, boolean applyOffset) {
